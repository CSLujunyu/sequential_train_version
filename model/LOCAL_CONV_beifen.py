...
class LOCAL_CONV(object):
    ...
    def sentiment_scoring(self, sent_emb, sent_len, sent_att):
        """
        ...
        self.local_att_mask = self.generate_att_mask(sent_lstm, sent_len, attr_c)
        # sent_conv :(batch*rev, attr, channel)
        sent_conv = tf.einsum('bik,bai->bak', sent_conv, self.local_att_mask)
    ...
    def init_attribute(self):
        ...
        attr_c_v = tf.Variable(self.attr_c, name='attr_c_v', trainable=True)
    ...
    def generate_att_mask(self, sent, sent_len, context, mask_value=-2 ** 32 + 1):
        ...
        return tf.nn.softmax(att * mask + mask_value * (1-mask),axis=-1)
