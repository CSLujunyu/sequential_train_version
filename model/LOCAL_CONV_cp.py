...
class LOCAL_CONV(object):
    ...
    def sentiment_scoring(self, sent_emb, sent_len, sent_att, is_training):
        ...
        # sent_trans:(batch*rev, attr, emb)
        sent_trans = self.Local_Sent_Multi_Head_Transformer(sent_emb,self.local_pos_mask,name='sent_trans')

        # sent_lstm :(batch*rev, attr, emb)
        sent_lstm = tf.einsum('bik,bai->bak', sent_lstm, self.local_att_mask)

        # sent_conv:(batch, rev, attr, emb)
        sent_trans = tf.reshape(sent_trans, shape=[-1, self.conf['max_rev_len'], sent_trans .shape[-2], sent_trans .shape[-1]])
        # sent_conv:(batch, attr, rev, emb)
        sent_trans = tf.transpose(sent_trans, perm=[0, 2, 1, 3])
        # sent_lstm:(batch, rev, attr, emb)
        ...
        sent_trans = tf.einsum('bark,bar->bak', sent_trans, sent_att)
